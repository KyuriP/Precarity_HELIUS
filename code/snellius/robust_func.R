# Load libraries
source("helius/code/libraries.R")   
    ...
    for (j in 1:num_variables) {
      if (edge_type_count$none[i, j] / num_subsamples <= (1 - threshold)) {
        ...
        if (algorithm %in% c("FCI", "CCI")) {
          ...
          # For PC: determine which edge type (0 or 1) is the majority based on threshold
          if (edge_type_count$arrowhead[i, j] / num_subsamples >= threshold) {
