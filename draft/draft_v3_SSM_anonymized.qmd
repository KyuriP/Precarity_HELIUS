...
execute:
  ...
  warning: false
abstract: |
  \noindent \textcolor{BrickRed}{Socioeconomic precariousness and depression are closely linked, yet the directional structure and the conditions under which feedback may matter for intervention trajectories remain insufficiently understood. Using cross-sectional data from more than 21,000 adults in the multi-ethnic HELIUS study, we applied cycle-capable constraint-based causal discovery methods that allow for latent confounding and feedback to estimate candidate graph structures linking financial stress, domain-specific precariousness, and depressive symptoms. Because the data are cross-sectional and many edge orientations are not uniquely determined, we summarize results as stability patterns across specifications and resampling, and we treat orientations as hypothesis-generating rather than as identified causal directionality. We then developed a calibrated simulation framework to examine how depression and precariousness could co-evolve under stress-reducing scenarios across alternative plausible feedback regimes that are compatible with the observed data. Across specifications and algorithms, financial stress is the most consistently adjacent factor to depression. Directional edge marks are less stable: in some FCI summaries the endpoint pattern is more often compatible with financial stress preceding depression in the PAG (ancestral-constraint) sense, whereas CCI and PC more often leave the same relation directionally unresolved; we therefore treat direction as hypothesis-generating and partly indeterminate. Several symptom-level configurations are also consistent with depressive symptoms contributing to downstream social precariousness. In scenario simulations, stronger internal feedback produces slower but more persistent improvement following reductions in financial stress, whereas weaker feedback produces faster but less durable change. These results suggest that intervention trajectories may depend not only on the magnitude of external support but also on the feedback architecture linking economic stress and mental health, and they motivate future longitudinal and quasi-experimental work to clarify directionality and quantify policy-relevant effects.}
---
...
\textcolor{BrickRed}{Throughout, “interventions” on recent financial stressors (\textit{S.fin}), social precariousness (\textit{P.soc}), and overall precariousness refer to interventions on underlying conditions that would be expected to shift these measured proxies. For \textit{S.fin}, this includes policies and programs that reduce acute financial strain, such as income support or benefits, debt relief, bill assistance, or support with financial administration. For \textit{P.soc}, this includes interventions that increase social support or reduce barriers to participation, such as social prescribing, community outreach, or access to support services. An intervention on overall precariousness corresponds to a multi-component package that shifts more than one domain. Interpreting changes in these proxies as policy-relevant shifts requires additional assumptions: (i) the intervention produces a stable change in the proxy as measured; (ii) the proxy is measured comparably across relevant groups and settings; (iii) the intervention does not have large effects on depression through pathways that bypass the targeted proxy; and (iv) the intervention effect is well-defined at the level of the proxy, so that a given change in the proxy corresponds to a meaningful change in underlying conditions.}
...
\textcolor{BrickRed}{To characterize candidate causal structures and directional constraints between precariousness and depression, we use constraint-based causal discovery algorithms that allow for feedback loops and latent confounding.} Specifically, we apply the *Fast Causal Inference* (FCI) and *Cyclic Causal Inference* (CCI) algorithms, \textcolor{BrickRed}{both of which allow latent confounding and can represent certain classes of feedback/cyclicity under additional assumptions} [@mooij2020constraint; @strobl2019].
...
To address this, we use the Randomized Conditional Correlation Test (RCoT), a \textcolor{BrickRed}{scalable nonparametric alternative that approximates kernel methods using random Fourier features, replacing expensive kernel-matrix computations with lower-dimensional feature operations and thereby substantially reducing runtime at large sample sizes} [@strobl2019approximate; @zhang2012kernel]. \textcolor{BrickRed}{RCoT improves computational feasibility relative to KCIT and allows sensitivity analyses at HELIUS sample size, but it does not remove the fundamental loss of statistical power that typically occurs as the conditioning set size $|Z|$ increases.} For further details, see @sec-rcot.
...
\textcolor{BrickRed}{To summarize the stability of the estimated graph structure between depression and precariousness variables, we implement a comprehensive bootstrapped causal discovery procedure.} This procedure systematically varies key analysis parameters across a grid of configurations, enabling us to assess the stability of inferred relationships under different settings.
...
2. **Across conditions**, \textcolor{BrickRed}{we summarize edge marks by their selection frequencies across the specification grid. If no single edge mark is most frequent (i.e., the top frequencies are tied), we label the edge as uncertain and draw it as a dashed line in the final graph} (Step 3 in @fig-workflow). \textcolor{BrickRed}{Dashed edges therefore indicate indeterminacy across specifications and are not treated as directionally informative.}

This summarization process is repeated separately for each causal discovery algorithm (FCI, CCI, PC), \textcolor{BrickRed}{yielding a single summary graph per algorithm. These final graphs represent only the most stable and consistent relationships across a wide range of plausible parameterizations, improving interpretability and making sensitivity to analysis choices explicit.}
...
```{r}
#| label: fig-concept
#| fig-cap: '***High-level schematic of the dynamic model structure used in simulation.*** Arrows represent influence coefficients $\alpha$, matching the terms in the system of stochastic differential equations. $D_i(t)$ and $P_i(t)$ denote depressive symptoms and social precariousness for individual $i$, with $S_i$ representing static financial stress. \textcolor{BrickRed}{In the simulation model, financial stress is specified as an external input to both depressive symptoms and social precariousness, and depression and precariousness are specified to be mutually coupled.} Arrows depict influence pathways that are operationalized in our quantitative model. \textcolor{BrickRed}{This abstraction is informed by the causal discovery results and provides the structure for the simulation experiments.}'
#| fig-align: center
#| out-width: 40%
...
\textcolor{BrickRed}{Because the estimated graphs are equivalence-class outputs and HELIUS is cross-sectional, we interpret these results as hypothesis-generating constraints. We distinguish adjacency stability, meaning whether a connection is repeatedly recovered, from orientation stability, meaning how consistently endpoints are oriented across specifications. Unresolved orientations are treated as indeterminacy rather than as evidence for or against a directional effect. When stability is limited, we report the relation as indeterminate and refrain from directional interpretation.}
...
@fig-sum provides a summary of how precariousness factors collectively influence and are influenced by overall depression severity, \textcolor{BrickRed}{focusing on the estimated graph structure between precariousness factors (\textit{P.hou, P.emp, P.soc, S.rel, S.fin}) and the depression sum score (PHQsum), as returned by the FCI algorithm.}
...
\textcolor{BrickRed}{In panel (a) we see that employment precariousness (\textit{P.emp}) and social precariousness (\textit{P.soc}) show limited directional support toward depression, whereas financial stress (\textit{S.fin}) shows the most stable adjacency with \textit{PHQsum} and is more often oriented in a direction compatible with \textit{S.fin} preceding \textit{PHQsum}.} While *P.emp* and *P.soc* are generally not recognized as causes of other precariousness factors, *S.fin* emerges as a potential cause, as indicated by its circle edge endpoint. \textcolor{BrickRed}{This pattern is summarized in the edge-type proportion matrix shown in panel (b). Panel (b) reports \emph{selection frequencies} of edge marks across bootstrap resamples (and specifications), not p-values or significance levels; values close to 0 or 1 indicate stability of the recovered mark under the procedure.}
...
<!-- We therefore treat disagreement as a substantive signal that direction is not reliably resolved here, and we restrict interpretation to patterns that remain stable across methods rather than privileging one algorithm’s orientation.  -->
<!-- Where directional constraints do appear, they should be read in the equivalence-class/ancestral-constraint sense (e.g., a PAG mark indicating that one variable is not an ancestor of the other), not as an identified causal effect or effect size. We therefore treat any directional tendency involving \textit{S.fin} and depression as hypothesis-generating rather than conclusive, and we view the stable \textit{S.fin}--depression connection as motivating the hypothesis that economic stressors may contribute to broader cycles of psychosocial risk.} -->
<!-- \textcolor{BrickRed}{In this framing, the causal discovery stage is used to identify which structural motifs are consistently supported across specifications (and to make explicit where direction collapses into indeterminacy), and the simulation stage then explores how qualitatively different feedback regimes, compatible with the observed moments and those stable motifs, translate into different intervention trajectories.} -->
...
Although our findings are based on Amsterdam-specific data, they align with research in other high-income urban settings, where structural inequalities such as poverty, unemployment, and housing insecurity consistently predict poor psychological outcomes [@fryers2003social; @weich1998poverty]. \textcolor{BrickRed}{While local contexts may alter specific pathways, these parallels suggest that similar mechanisms could operate elsewhere; replication in other populations is needed to assess generalizability.}
...
Our own simulations support this view: while all models converged to similar long-term outcomes under sustained support, \textcolor{BrickRed}{differences in internal structure, particularly the assumed direction and strength of coupling between precariousness and depression, produced systematic variation in how quickly and through which pathways systems responded to intervention.}
...
\textcolor{BrickRed}{Because the intervention variables are measured proxies and the data are cross-sectional, the simulations should be interpreted as scenario analyses of proxy shifts rather than as identified effects of specific policy interventions.}
...
\textcolor{BrickRed}{Here, we provide a short primer for interpreting PAG/MAAG/CPDAG outputs and edge marks used in the Results. Readers who want a more detailed tutorial-style introduction and simulation-based demonstrations of finite-sample behavior can consult} @park2024discovering.
...
\textcolor{BrickRed}{Importantly, these endpoint marks encode \emph{ancestral constraints} within an equivalence class (e.g., "B is not an ancestor of A"), and should not be interpreted as identified causal effects or effect sizes from A to B.}
