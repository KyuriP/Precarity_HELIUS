...
execute:
  ...
  warning: false
abstract: |
  \noindent Socioeconomic precariousness and depression are closely linked, yet the mechanisms through which they reinforce each other and shape responses to intervention remain poorly understood. Using data from more than 21,000 adults in the multi-ethnic HELIUS study, we applied causal discovery methods to identify directional relationships between financial stress, domain-specific precariousness, and depressive symptoms. We then developed a simulation framework that translates these causal estimates into a dynamic model capable of testing how interventions might alter system behavior. The analysis identifies financial stress as a plausible upstream driver of depression, but also shows that feedback from depressive symptoms to social precariousness can slow or destabilize recovery. Simulated interventions indicate that systems with stronger internal feedbacks exhibit slower but more persistent improvements, whereas weaker feedbacks produce rapid yet short-lived change. These findings suggest that the success of social or psychological interventions depends not only on the external support provided but also on the internal feedback architecture linking economic stress and mental health. More broadly, the study illustrates how computational modeling can help explain why structural disadvantage and depression remain mutually reinforcing, informing strategies for resilience and equity in population mental health.
---
...
[To] characterize candidate causal structures and directional constraints between precariousness and depression, we use constraint-based causal discovery algorithms that allow for feedback loops and latent confounding.]{color="red"} Specifically, we apply the *Fast Causal Inference* (FCI) and *Cyclic Causal Inference* (CCI) algorithms, both of which can detect cycles and non-acyclic structures [@mooij2020constraint; @strobl2019]. 
...
```{r}
#| label: fig-concept
#| fig-cap: "***High-level schematic of the dynamic model structure used in simulation.*** Arrows represent influence coefficients $\\alpha$, matching the terms in the system of stochastic differential equations. $D_i(t)$ and $P_i(t)$ denote depressive symptoms and social precariousness for individual $i$, with $S_i$ representing static financial stress. Financial stress acts as an upstream driver of depressive symptoms and social precariousness, pushing both up. Depression and precariousness influence each other bidirectionally, forming a reinforcing feedback loop. Arrows depict influence pathways that are operationalized in our quantitative model. This abstraction summarizes dependencies identified by causal discovery and anchors our intervention experiments."
#| fig-align: center
#| out-width: 40%
