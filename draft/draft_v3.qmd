...
csl: nature.csl
abstract: |
  \noindent Mental health outcomes are shaped not only by individual vulnerabilities but also by structural conditions such as financial strain and precarious living circumstances. Understanding how these factors interact is essential for designing effective interventions. Previous research has established associations between socioeconomic stressors and depression, but the causal pathways and feedback loops that sustain vulnerability remain unclear. Here we investigate how the internal structure linking precariousness and depression shapes responsiveness to external support. Using cross-sectional data from more than 21,000 participants in the HELIUS study, we applied cycle-capable causal discovery to infer directional dependencies among financial stress, domain-specific precariousness, and depressive symptoms. Our analysis identifies financial stress as a likely upstream driver of depression, with symptoms such as sleep disturbance and depressed mood initiating broader social vulnerability. Embedding these insights in a calibrated dynamical model, we show that system responses to stress reduction depend on internal feedback strength: some configurations yield rapid but short-lived improvements, whereas others produce slower yet more persistent gains. These findings demonstrate that intervention outcomes depend not only on external support but also on hidden system properties. More broadly, this work illustrates how combining causal discovery with dynamical modelling can guide targeted strategies to improve mental health under socioeconomic stress.
  

  <!-- This study investigates how the internal structure of depression–precariousness dynamics shapes responsiveness to external support. Using cross-sectional data from the HELIUS study, we first apply cycle-capable causal discovery algorithms to estimate directional dependencies between financial stress, domain-specific precariousness, and depressive symptoms. Our multi-resolution analysis—spanning both aggregate and symptom-level constructs—highlights financial stress as a plausible upstream driver of depression, with symptoms such as sleep disturbance and depressed mood emerging as initiators of broader social vulnerability. -->
...
To address this question, we analyze cross-sectional data from more than 21,000 participants in the HELIUS (HEalthy LIfe in an Urban Setting) study [@stronks2013unravelling; @snijder2017cohort] using a two-step approach. First, we apply cycle-capable causal discovery algorithms to infer directional relationships among financial stress, domain-specific precariousness, and depressive symptoms. By examining both aggregated measures (a depression sum score and a composite precariousness index) and disaggregated indicators (individual symptoms and life-domain factors), we capture both broad patterns and fine-grained pathways. Second, we translate these insights into a computational dynamical model that simulates how depression and precariousness co-evolve under varying levels of external stress. This framework enables us to examine how system properties—such as feedback strength and stochastic noise—shape responsiveness to intervention.


We distil these results into a simple systems view (@fig-concept). Financial stress acts as an upstream driver that raises both depressive symptoms and social precariousness. In turn, precariousness and depression influence one another in both directions, forming a reinforcing feedback loop that can sustain or amplify vulnerability. This minimal structure clarifies the policy levers we analyse—relieving financial stress to simultaneously reduce precariousness and treat depression—and makes a concrete prediction: the impact of any lever depends not only on its magnitude but on the feedback architecture of the system. In what follows, we identify this structure and operationalize it in a stochastic dynamical model to examine how feedback strength and noise shape responsiveness to intervention and the distribution of outcomes.

<!-- @fig-concept presents this high-level model, derived from the causal discovery analysis and serving as the foundation for our simulation experiments. It highlights financial stress ($S_i$) as an upstream driver influencing both depressive symptoms ($D_i$) and social precariousness ($P_i$). Depression contributes to broader social vulnerability by feeding into precariousness, while precariousness feeds back into depression, creating reinforcing cycles. This framework illustrates that intervention effects depend not only on external inputs but also on the internal feedback architecture of the system. Full methodological details, including data processing, algorithmic specifications, and model calibration procedures, are provided in the [Methods](#sec-methods) section. -->
...
```{r}
#| label: fig-concept
#| fig-cap: "***Resulting high-level causal structure guiding intervention simulations.*** Financial stress acts as an upstream driver of depressive symptoms and social precariousness, pushing both up. Depression and precariousness influence each other bidirectionally, forming a reinforcing feedback loop. Arrows depict influence pathways that are operationalized in our quantitative model. This abstraction summarizes dependencies identified by causal discovery and anchors our intervention experiments. Full data processing, algorithms, and calibration are described in [Methods](#sec-methods). Financial stress sits “upstream”: it pushes up both depressive symptoms and social precariousness. Depression and precariousness then affect each other in both directions, creating a feedback loop that can trap people in escalating vulnerability. This structure helps explain why policies acting at different points—relieving financial stress, treating depression, or improving social stability—can have very different downstream effects."
#| fig-align: center
#| out-width: 50%


knitr::include_graphics("img/conceptmodel_withoutnotation.pdf")
```
...
We present the results in two parts. First, we report the causal structures inferred from the HELIUS data at the levels of aggregate depression, individual symptoms, and aggregated precariousness. Second, we embed these findings in a simplified dynamical model to simulate how alternative feedback structures shape the system’s response to stress-reducing interventions.

## Causal Structure Linking Precariousness and Depression

### Depression as sum score
@fig-sum provides a summary of how precariousness factors collectively influence and are influenced by overall depression severity, focusing on the causal relationships between precariousness factors (*P.hou*, *P.emp*, *P.soc*, *S.rel*, *S.fin*) and the depression sum score (*PHQsum*), as identified by the FCI algorithm.
...
#| layout-ncol: 2
#| label: fig-sum
#| fig-cap: "***Causal pathways between precariousness factors and overall depression severity.*** Analysis with the FCI algorithm indicates that financial stress (*S.fin*) is the most consistent upstream influence on depression severity (*PHQsum*), while other forms of precariousness show weaker or more ambiguous connections. **(a)** Aggregated causal graph showing the most frequent edge types across bootstraps. The edge pattern highlights *S.fin* as a plausible driver of depression, whereas employment and social precariousness are more often positioned downstream. Dashed edges indicate ties, with a bold horizontal bar marking where the tie occurs.  **(b)** Proportion matrix summarizing edge types across all bootstraps: each matrix cell $[i, j]$ indicates how often a given edge from variable $i$ to variable $j$ exhibited a particular edge mark (e.g., arrowhead, tail, or circle) adjacent to $j$. Green indicates arrowheads, coral arrowtails, blue circle marks, and gray absence of an edge. Colors are blended and shaded by frequency, with dominant symbols appearing more opaque. For example, matrix[$\\mathit{S.fin}$, $\\mathit{PHQsum}$] shows a green arrowhead ($>$) and matrix[$\\mathit{PHQsum}$, $\\mathit{S.fin}$] shows a coral arrowtail (\\textemdash), together indicating that $\\mathit{S.fin} \\rightarrow \\mathit{PHQsum}$ in the majority of bootstrapped graphs."
#| fig-subcap: 
#|   - "FCI PAG"
#|   - "Proportion matrix"
#| fig-align: center
...
Within this framework, sleep disturbance emerged as a potentially initiating symptom, with directed connections to both symptom-level and precariousness variables—mirroring longitudinal findings that identify sleep disruption as an early signal for depressive episodes and social withdrawal [@zawadzki2013rumination; @alvaro2013systematic].
Depressed mood, another core symptom, occupied a central position in the network. Not only was it influenced by upstream symptoms like anhedonia and guilt, but it also showed directional influence on social precariousness—suggesting how internal psychological states may actively shape external vulnerability. The CCI algorithm further identified a potential feedback loop between depressed mood and overall precariousness, resonating with social drift theories in depression, which posit that declining mental health can lead to social exclusion, job instability, and housing insecurity, reinforcing depressive cycles [@compton2015social; @boschloo2015network].
...
Our results also highlight why single-point interventions may fail to produce lasting change. The broader causal structure reveals multiple interlocking feedback loops spanning both psychological symptoms and structural risk factors, which may re-amplify vulnerability even after temporary relief. Supporting this, recent simulation work shows that feedback loops, even when modest in strength, can substantially diminish the long-term impact of isolated interventions unless multiple reinforcing links are simultaneously disrupted [@park2025role].
Our own simulations support this view: while all models converged to similar long-term outcomes under sustained support, differences in internal structure—particularly the dominant direction of coupling between precariousness and depression—produced systematic variation in how quickly and through which pathways systems responded to intervention. Stronger coupling from precariousness to depression slowed recovery and prolonged depression, showing that internal architecture can be as important as external input.
