...
\maketitle
\begin{abstract}
\noindent Mental health outcomes are shaped not only by individual
vulnerabilities but also by structural conditions such as financial
strain and precarious living circumstances. Understanding how these
factors interact is essential for designing effective interventions.
Previous research has established associations between socioeconomic
stressors and depression, but the causal pathways and feedback loops
that sustain vulnerability remain unclear. Here we investigate how the
internal structure linking precariousness and depression shapes
responsiveness to external support. Using cross-sectional data from more
than 21,000 participants in the HELIUS study, we applied cycle-capable
causal discovery to infer directional dependencies among financial
stress, domain-specific precariousness, and depressive symptoms. Our
analysis identifies financial stress as a likely upstream driver of
depression, with symptoms such as sleep disturbance and depressed mood
initiating broader social vulnerability. Embedding these insights in a
calibrated dynamical model, we show that system responses to stress
reduction depend on internal feedback strength: some configurations
yield rapid but short-lived improvements, whereas others produce slower
yet more persistent gains. These findings demonstrate that intervention
outcomes are shaped not only by external support but also by the
system's internal feedback architecture. More broadly, this work
illustrates how combining causal discovery with dynamical modelling can
guide targeted strategies to improve mental health under socioeconomic
stress.
\end{abstract}
...
study (Snijder et al., 2017; Stronks et al., 2013) using a two-step
approach. First, we apply cycle-capable causal discovery algorithms to
infer directional relationships among financial stress, domain-specific
precariousness, and depressive symptoms. By examining both aggregated
measures (a depression sum score and a composite precariousness index)
and disaggregated indicators (individual symptoms and life-domain
factors), we capture both broad patterns and fine-grained pathways.
Second, we translate these insights into a computational dynamical model
that simulates how depression and precariousness co-evolve under varying
levels of external stress. This framework enables us to examine how
system properties, specifically feedback strength and stochastic noise,
shape responsiveness to intervention.

We distil these results into a simple systems view
(Figure~\ref{fig-concept}). Financial stress acts as an upstream driver
that raises both depressive symptoms and social precariousness. In turn,
precariousness and depression influence one another in both directions,
forming a reinforcing feedback loop that can sustain or amplify
vulnerability. This minimal structure clarifies the policy levers we
analyse; relieving financial stress to simultaneously reduce
precariousness and treat depression---and makes a concrete prediction:
the impact of any lever depends not only on its magnitude but on the
feedback architecture of the system. In what follows, we identify this
structure and operationalize it in a stochastic dynamical model to
examine how feedback strength and noise shape responsiveness to
intervention and the distribution of outcomes.

\begin{figure}

\centering{

\includegraphics[width=0.5\textwidth,height=\textheight]{img/conceptmodel_withoutnotation.pdf}

}

\caption{\label{fig-concept}\textbf{\emph{Resulting high-level causal
structure guiding intervention simulations.}} Financial stress acts as
an upstream driver of depressive symptoms and social precariousness,
pushing both up. Depression and precariousness influence each other
bidirectionally, forming a reinforcing feedback loop. Arrows depict
influence pathways that are operationalized in our quantitative model.
This abstraction summarizes dependencies identified by causal discovery
and anchors our intervention experiments. Full data processing,
algorithms, and calibration are described in
\hyperref[sec-methods]{Methods}.}
...
\caption{\label{fig-sum}\textbf{\emph{Causal pathways between
precariousness factors and overall depression severity.}} Analysis with
the FCI algorithm indicates that financial stress (\emph{S.fin}) is the
most consistent upstream influence on depression severity
(\emph{PHQsum}), while other forms of precariousness show weaker or more
ambiguous connections. \textbf{(a)} Aggregated causal graph showing the
most frequent edge types across bootstraps. The edge pattern highlights
\emph{S.fin} as a plausible driver of depression, whereas employment and
social precariousness are more often positioned downstream. Dashed edges
indicate ties, with a bold horizontal bar marking where the tie occurs.
\textbf{(b)} Proportion matrix summarizing edge types across all
bootstraps: each matrix cell \([i, j]\) indicates how often a given edge
from variable \(i\) to variable \(j\) exhibited a particular edge mark
(e.g., arrowhead, tail, or circle) adjacent to \(j\). Green indicates
arrowheads, coral arrowtails, blue circle marks, and gray absence of an
edge. Colors are blended and shaded by frequency, with dominant symbols
appearing more opaque. For example, matrix{[}\(\mathit{S.fin}\),
\(\mathit{PHQsum}\){]} shows a green arrowhead (\(>\)) and
matrix{[}\(\mathit{PHQsum}\), \(\mathit{S.fin}\){]} shows a coral
arrowtail (\textemdash), together indicating that
...
sleep disruption as an early signal for depressive episodes and social
withdrawal (Alvaro et al., 2013; Zawadzki et al., 2013). Depressed mood,
another core symptom, occupied a central position in the network. Not
only was it influenced by upstream symptoms like anhedonia and guilt,
but it also showed directional influence on social
precariousness---suggesting how internal psychological states may
...
worsening precariousness. Similarly, targeting symptoms that appear
embedded in potential feedback loops linking psychological and social
domains, such as depressed mood, may offer strategic leverage for
breaking mutually reinforcing cycles of mental decline and social
isolation. Importantly, interventions targeting such cycles may have
...
employment engagement, and housing stability, particularly when combined
with social or financial supports (Lund et al., 2018; McDaid et al.,
2019). These dual benefits underscore the value of integrated,
cross-domain strategies that simultaneously address internal symptoms
and external stressors, and show promise of the external validity of our
analysis.
...
temporary relief. Supporting this, recent simulation work shows that
feedback loops, even when modest in strength, can substantially diminish
the long-term impact of isolated interventions unless multiple
reinforcing links are simultaneously disrupted (Park, Li, et al., 2025).
Our own simulations support this view: while all models converged to
similar long-term outcomes under sustained support, differences in
internal structure---particularly the dominant direction of coupling
between precariousness and depression---produced systematic variation in
how quickly and through which pathways systems responded to
intervention. Stronger coupling from precariousness to depression slowed
recovery and prolonged depression, showing that internal architecture
can be as important as external input. This supports growing calls for
...
In this light, understanding causal architecture is not merely
theoretical---it is foundational to designing more effective and
...
and small effect sizes are harder to detect. Moreover, some of the
relations showed sensitivity to the choice of conditional independence
(CI) test. The Gaussian CI test, which assumes linearity and Gaussian
noise, often produced denser graphs, likely due to overestimating weak
or noisy linear associations. In contrast, RCoT, a nonparametric
alternative, avoids these assumptions but relies on a Gaussian radial
basis function (RBF) kernel to assess dependence. While flexible in
capturing nonlinear relationships, this kernel is optimized for smooth,
continuous variables and may underperform with discrete or ordinal data.
Its similarity estimates can be unreliable when applied to variables
with discontinuities, potentially leading to missed dependencies. This
is particularly relevant in the HELIUS dataset, where some variables are
...
\includegraphics[width=0.9\textwidth,height=\textheight]{draft_v3_anonymized_psych_files/figure-pdf/fig-dist-1.pdf}
...
\includegraphics{img/param_tradeoff.pdf}
...
simulation are available at \textless{}\emph{anonymized}\textgreater.
The repository contains documented R code and instructions for
reproducing the analyses reported in this manuscript.
...
\begin{center}
\includegraphics{draft_v3_anonymized_psych_files/figure-pdf/unnamed-chunk-12-1.pdf}
\end{center}
...
\begin{center}
\includegraphics[width=0.55\textwidth,height=\textheight]{draft_v3_anonymized_psych_files/figure-pdf/unnamed-chunk-13-1.pdf}
\end{center}

\begin{center}
\includegraphics[width=0.7\textwidth,height=\textheight]{draft_v3_anonymized_psych_files/figure-pdf/unnamed-chunk-14-1.pdf}
\end{center}
...
\begin{center}
\includegraphics[width=0.7\textwidth,height=\textheight]{draft_v3_anonymized_psych_files/figure-pdf/unnamed-chunk-15-1.pdf}
\end{center}
...
\begin{center}
\includegraphics{draft_v3_anonymized_psych_files/figure-pdf/unnamed-chunk-16-1.pdf}
\end{center}
...
\begin{center}
\includegraphics[width=0.7\textwidth,height=\textheight]{draft_v3_anonymized_psych_files/figure-pdf/unnamed-chunk-17-1.pdf}
\end{center}
...
\begin{center}
\includegraphics[width=0.7\textwidth,height=\textheight]{draft_v3_anonymized_psych_files/figure-pdf/unnamed-chunk-18-1.pdf}
\end{center}
...
\begin{minipage}{0.50\linewidth}
\begin{center}
\includegraphics{draft_v3_anonymized_psych_files/figure-pdf/unnamed-chunk-19-1.pdf}
\end{center}
\end{minipage}%
%
\begin{minipage}{0.50\linewidth}
\begin{center}
\includegraphics{draft_v3_anonymized_psych_files/figure-pdf/unnamed-chunk-19-2.pdf}
\end{center}
\end{minipage}%
\newline
\begin{minipage}{0.50\linewidth}
\begin{center}
\includegraphics{draft_v3_anonymized_psych_files/figure-pdf/unnamed-chunk-19-3.pdf}
\end{center}
\end{minipage}%
%
\begin{minipage}{0.50\linewidth}
\begin{center}
\includegraphics{draft_v3_anonymized_psych_files/figure-pdf/unnamed-chunk-19-4.pdf}
\end{center}
\end{minipage}%
...
\begin{center}
\includegraphics[width=0.6\textwidth,height=\textheight]{draft_v3_anonymized_psych_files/figure-pdf/unnamed-chunk-20-1.pdf}
\end{center}
...
\begin{center}
\includegraphics{draft_v3_anonymized_psych_files/figure-pdf/unnamed-chunk-21-1.pdf}
\end{center}

\begin{center}
\includegraphics{draft_v3_anonymized_psych_files/figure-pdf/unnamed-chunk-22-1.pdf}
\end{center}
