...
execute:
  ...
  warning: false
abstract: |
  \noindent \textcolor{BrickRed}{Socioeconomic precariousness and depression are closely linked, yet the directional structure of this relationship, and the conditions under which feedback may shape intervention trajectories, remain unclear. Using cross-sectional data from more than 21,000 adults in the multi-ethnic HELIUS study, we applied constraint-based causal discovery methods that allow for latent confounding and feedback to estimate candidate graph structures linking financial stress, domain-specific precariousness, and depressive symptoms. Because the data are cross-sectional and many edge orientations are not uniquely determined, we summarize the results as stability patterns across specifications and resampling and treat edge directions as tentative, data-compatible hypotheses rather than identified causal effects. We then use a calibrated simulation model to examine how depression and precariousness could co-evolve under stress-reducing scenarios across different feedback strengths compatible with the observed data. Across specifications and algorithms, financial stress is the factor most consistently linked to depression in the graph summaries. Directional edge marks are less stable: in some FCI summaries, the endpoint pattern on this edge is more often consistent with financial stress preceding depression, whereas CCI and PC more often leave the same relation unresolved. We therefore treat the suggested direction as tentative and partly indeterminate. Several symptom-level configurations are also consistent with depressive symptoms contributing to downstream social precariousness. In the simulation model, slower but more persistent improvement follows reductions in financial stress under stronger internal feedback, whereas faster but less durable change follows under weaker feedback. These results suggest that intervention trajectories may depend not only on the magnitude of external support but also on the feedback architecture linking economic stress and mental health. They motivate future longitudinal and quasi-experimental work to clarify directionality, estimate feedback strengths, and assess the policy relevance of these patterns.}
---
