...
Despite this, the skeleton structure and overall causal directions derived from CCI align well with the results of the FCI algorithm. For example, both algorithms identify directional pathways from the stressors (*S.rel*, *S.fin*) to *P.soc*, and suggest that *P.soc* is not a cause of *PHQsum*, but more likely affected by it. *P.emp* is not inferred to cause any of its neighboring variables (*P.hou*, *PHQsum*, *S.fin*), and instead appears to be affected by them. *P.hou*, in turn, is connected to *P.emp* and *S.fin* with circle endpoints, indicating that it may be a potential source of influence rather than an outcome. These consistent pathways underscore the robustness of the core relational architecture across algorithms amid variation in directional confidence.
...
Moving from the sum score representation to the disaggregated symptom-level graph provides a more granular perspective on the causal relationships between precariousness factors and depression symptoms. Unlike the sum score graph, which aggregates all symptoms into a single measure, the symptom-level graph reveals the heterogeneity in how specific symptoms (*con* (concentration), *slp* (sleep), *ene* (energy), *app* (appetite), *mot* (motor), *sui* (suicidal), *anh* (anhedonia), *glt* (guilt), and *dep* (depressed mood)) are influenced by, and in turn influence, different forms of precariousness. 
...
Other precariousness factors exhibit more uncertainty but still notable relationships. *S.rel* shows weak but existing connections with *slp*, *glt*, and *app*, though directionality remains unclear in many cases. *P.emp* also connects to symptoms like *slp*, *mot*, and *con*, but, like *S.rel*, these relationships exhibit a 50/50 split in directionality, reflecting uncertainty in the inferred causal paths.
...
For each model, we generate a virtual population of 1,000 individuals and simulate an external intervention that reduces financial stress ($S$), which acts as a shared input across individuals. This intervention defines the control input, while different values of $\alpha_{DP}$ represent alternative internal feedback scenarios.
...
@fig-intervention summarizes how variation in $\alpha_{DP}$ shapes system-level responses. Panel A shows the mean levels of depression ($D$) and precariousness ($P$) at time steps 100, 300, and 500, with the x-axis indicating the strength of external support, defined as the proportional reduction of $S$ toward its minimum observed value.
Each line represents a different scenario with a different $\alpha_{DP}$, color-coded from low (purple) to high (yellow).

In the early phase of intervention ($t = 100$), depression and precariousness exhibit contrasting patterns. Depression declines more rapidly in scenarios with lower $\alpha_{DP}$, where its dynamics are driven more directly by the external stressor $S$. 
...
Despite these short-term differences, all scenarios ultimately converge to similar steady-state levels under sustained intervention. By time step 300, convergence is already substantial—though some variation remains, particularly in $P$, where gradients between models are still visible. By step 500, trajectories have nearly fully aligned across all configurations, consistent with the shared equilibrium structure of the linear system. This convergence highlights that the key distinctions lie not in whether intervention is effective, but in how quickly and through which pathways change unfolds.

Panel B shows full time trajectories for two representative cases: one with minimal internal feedback ($\alpha_{DP} = 0$) and one at the upper admissible limit ($\alpha_{DP} = 0.65$). In both, intervention begins at step 300 and ends at step 1500. When $\alpha_{DP} = 0$, depression responds rapidly to the reduction in stress and rebounds shortly after support is withdrawn. In contrast, when $\alpha_{DP}$ is high, $D$ responds more gradually and recovers more slowly—reflecting its increased dependence on the slower-changing $P$.
Precariousness declines in both scenarios during intervention and returns to baseline afterward, but with a slightly sharper drop in the higher $\alpha_{DP}$ condition. This occurs because $P$ becomes more directly responsive to $S$, as $\alpha_{PS}$ increases with $\alpha_{DP}$.
...
The causal analysis consistently identified financial stress as a key plausible driver of depression, appearing across both aggregated and disaggregated models. 
At the symptom level, several nodes—sleep disturbance, depressed mood, guilt, and anhedonia—emerged as especially sensitive to external influences. Among these, depressed mood played a particularly central role: it was influenced by other symptoms and, in turn, showed directional influence on social precariousness, suggesting a feedback mechanism wherein worsening mood fosters social withdrawal, which may subsequently exacerbate depressive symptoms. 
The CCI algorithm further suggested a potential cyclic link between depressed mood and overall precariousness, hinting that symptom and precariousness factors may co-evolve in mutually reinforcing ways.
...
In contrast, social precariousness more often appeared as an effect rather than a cause—receiving arrows from symptoms like depressed mood and guilt, but rarely sending them. This asymmetry implies that social precariousness may often reflect downstream consequences of worsening mental health rather than being its root cause [@lund2010poverty; @compton2015social]. While such relationships are complex and likely reciprocal over time, the directional patterns observed provide a useful foundation for hypothesis generation and targeted intervention.

Taken together, these patterns offer actionable insights for intervention. Symptoms such as sleep disturbance, which show early and wide-reaching influence within the symptom network, may serve as effective targets for prevention—limiting escalation into broader depressive states or worsening precariousness. Similarly, depressed mood, which functions as a central hub influencing both symptom and precariousness variables, may offer a leverage point for disrupting reinforcing cycles of mental health decline and social isolation.
At the same time, the broader causal structure points to interconnected feedback loops that span both psychological symptoms and external conditions such as financial and social precariousness—factors that often reflect the influence of structural inequalities (e.g., income instability, housing insecurity, or social marginalization). These complex dynamics suggest that single-point interventions may be insufficient to produce lasting change.
Supporting this, recent simulation work by @park2025role demonstrates that feedback loops—even when modest in strength—can substantially reduce the long-term impact of isolated interventions, as system dynamics tend to re-amplify vulnerability through alternate pathways. In line with emerging perspectives in systems-oriented mental health research [@borsboom2017network], our findings underscore the need for multi-pronged strategies that address not only internal symptom dynamics but also the broader socioeconomic conditions that sustain vulnerability. Intervening simultaneously at multiple points of reinforcement may be essential to shift the system out of a maladaptive attractor and toward more stable trajectories of well-being.
...
Simulation results showed that even with identical steady-state covariances, differences in internal structure produced systematic variation in the timing and profile of system responses to intervention. 
When the effect of precariousness on depression was weaker, depression responded more rapidly to reductions in financial stress, reflecting a direct coupling to the external input.
In contrast, as the effect of precariousness on depression increased and depression became more indirectly influenced—mediated by the slower-changing precariousness variable—its response was more gradual and persistent. Precariousness, while generally more directly responsive across all models, also showed subtle differences, with slightly faster early declines in configurations where it was more sensitive to external stress.
These results highlight that intervention effectiveness is shaped not just by external input (e.g., stress reduction) but also by internal system architecture—how tightly different elements are coupled and through what paths change propagates. One-size-fits-all approaches may therefore fall short. From a policy or clinical perspective, more tailored interventions—targeting early-acting symptoms in some cases, or sustaining support over time in others—may be needed to achieve durable change, particularly in populations facing layered psychosocial vulnerabilities.
