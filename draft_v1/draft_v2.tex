...
overall count of arrowheads when directions are inferred. Despite this,
the skeleton structure and overall causal directions derived from CCI
align well with the results of the FCI algorithm. For example, both
...
may be a potential source of influence rather than an outcome. These
consistent pathways underscore the robustness of the core relational
architecture across algorithms amid variation in directional confidence.
...
measure, the symptom-level graph reveals the heterogeneity in how
specific symptoms (\emph{con} (concentration), \emph{slp} (sleep),
\emph{ene} (energy), \emph{app} (appetite), \emph{mot} (motor),
\emph{sui} (suicidal), \emph{anh} (anhedonia), \emph{glt} (guilt), and
\emph{dep} (depressed mood)) are influenced by, and in turn influence,
...
external intervention that reduces financial stress (\(S\)), which acts
as a shared input across individuals. This intervention defines the
control input, while different values of \(\alpha_{DP}\) represent
alternative internal feedback scenarios. We examine system behavior in
two complementary ways: (1) by analyzing average levels of depression
and precariousness at selected time points following intervention onset,
and (2) by simulating full temporal trajectories under sustained
intervention followed by recovery, using two representative cases at the
extremes of the \(\alpha_{DP}\) range.

Figure~\ref{fig-intervention} summarizes how variation in
\(\alpha_{DP}\) shapes system-level responses. Panel A shows the mean
levels of depression (\(D\)) and precariousness (\(P\)) at time steps
100, 300, and 500, with the x-axis indicating the strength of external
support, defined as the proportional reduction of \(S\) toward its
minimum observed value. Each line represents a different scenario with a
different \(\alpha_{DP}\), color-coded from low (purple) to high
(yellow).
...
to similar steady-state levels under sustained intervention. By time
step 300, convergence is already substantial---though some variation
remains, particularly in \(P\), where gradients between models are still
visible. By step 500, trajectories have nearly fully aligned across all
...
upper admissible limit (\(\alpha_{DP} = 0.65\)). In both, intervention
begins at step 300 and ends at step 1500. When \(\alpha_{DP} = 0\),
depression responds rapidly to the reduction in stress and rebounds
shortly after support is withdrawn. In contrast, when \(\alpha_{DP}\) is
high, \(D\) responds more gradually and recovers more
slowly---reflecting its increased dependence on the slower-changing
\(P\). Precariousness declines in both scenarios during intervention and
returns to baseline afterward, but with a slightly sharper drop in the
higher \(\alpha_{DP}\) condition. This occurs because \(P\) becomes more
directly responsive to \(S\), as \(\alpha_{PS}\) increases with
\(\alpha_{DP}\).
...
disaggregated models. At the symptom level, several nodes---sleep
disturbance, depressed mood, guilt, and anhedonia---emerged as
especially sensitive to external influences. Among these, depressed mood
played a particularly central role: it was influenced by other symptoms
and, in turn, showed directional influence on social precariousness,
suggesting a feedback mechanism wherein worsening mood fosters social
withdrawal, which may subsequently exacerbate depressive symptoms. The
CCI algorithm further suggested a potential cyclic link between
depressed mood and overall precariousness, hinting that symptom and
precariousness factors may co-evolve in mutually reinforcing ways.
Symptom sleep disturbance also played a key initiating role. It showed
directed connections to both social and employment precariousness and
exhibited many outgoing edges within the symptom network. This suggests
the symptom sleep disturbance may function as an early warning signal or
entry point into the depression--precariousness cycle, making it a
promising target for preventive intervention. In contrast, social
precariousness more often appeared as an effect rather than a
cause---receiving arrows from symptoms like depressed mood and guilt,
but rarely sending them. This asymmetry implies that social
precariousness may often reflect downstream consequences of worsening
mental health rather than being its root cause (Compton \& Shim, 2015;
Lund et al., 2010). While such relationships are complex and likely
reciprocal over time, the directional patterns observed provide a useful
foundation for hypothesis generation and targeted intervention.

Taken together, these patterns offer actionable insights for
intervention. Symptoms such as sleep disturbance, which show early and
wide-reaching influence within the symptom network, may serve as
effective targets for prevention---limiting escalation into broader
depressive states or worsening precariousness. Similarly, depressed
mood, which functions as a central hub influencing both symptom and
precariousness variables, may offer a leverage point for disrupting
reinforcing cycles of mental health decline and social isolation. At the
same time, the broader causal structure points to interconnected
feedback loops that span both psychological symptoms and external
conditions such as financial and social precariousness---factors that
often reflect the influence of structural inequalities (e.g., income
instability, housing insecurity, or social marginalization). These
complex dynamics suggest that single-point interventions may be
insufficient to produce lasting change. Supporting this, recent
simulation work by Park, Li, et al. (2025) demonstrates that feedback
loops---even when modest in strength---can substantially reduce the
long-term impact of isolated interventions, as system dynamics tend to
re-amplify vulnerability through alternate pathways. In line with
emerging perspectives in systems-oriented mental health research
(Borsboom, 2017), our findings underscore the need for multi-pronged
strategies that address not only internal symptom dynamics but also the
broader socioeconomic conditions that sustain vulnerability. Intervening
simultaneously at multiple points of reinforcement may be essential to
shift the system out of a maladaptive attractor and toward more stable
...
variable and derived all other parameters accordingly, yielding a family
of valid models consistent with the data. Simulation results showed that
even with identical steady-state covariances, differences in internal
structure produced systematic variation in the timing and profile of
system responses to intervention. When the effect of precariousness on
depression was weaker, depression responded more rapidly to reductions
in financial stress, reflecting a direct coupling to the external input.
In contrast, as the effect of precariousness on depression increased and
depression became more indirectly influenced---mediated by the
slower-changing precariousness variable---its response was more gradual
and persistent. Precariousness, while generally more directly responsive
across all models, also showed subtle differences, with slightly faster
early declines in configurations where it was more sensitive to external
stress. These results highlight that intervention effectiveness is
shaped not just by external input (e.g., stress reduction) but also by
internal system architecture---how tightly different elements are
coupled and through what paths change propagates. One-size-fits-all
approaches may therefore fall short. From a policy or clinical
perspective, more tailored interventions---targeting early-acting
symptoms in some cases, or sustaining support over time in others---may
be needed to achieve durable change, particularly in populations facing
layered psychosocial vulnerabilities.
...
are sensitive to the system's internal architecture, particularly when
timely or rapid impact is a key concern.
...
real-world mental health policy and intervention design.
